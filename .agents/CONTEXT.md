...
- **Layout & Rendering Fixes**: Implemented a collapsible layout sidebar with a smooth rotate-to-X animated hamburger button. Restored video wallpaper visibility on the Dashboard route by clearing background fills in Three.js and CSS. Redesigned image attachments inside NoteCards to preserve aspect ratios (using object-contain with max-h-450px) instead of forced banner crops.
