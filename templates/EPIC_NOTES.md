...
- [ ] `updateNote/` -- инлайн-редактирование заметки
- [ ] `deleteNote/` -- кнопка удаления с подтверждением
...
- [ ] Создать заметку с медиа-вложением -> файл загрузился, превью отображается
- [ ] Добавить теги -> теги видны на карточке, фильтрация работает
- [ ] Закрепить заметку -> отображается вверху ленты
- [x] Удалить заметку -> исчезла из ленты
...
> Основной функционал реализован: CRUD заметок, загрузка медиа, теги, закрепление. Данные живут в БД и переживают перезапуск. Инлайн-редактирование и фильтрация по тегам -- следующая итерация.
...
- Пагинация реализована на бэкенде, фронтенд пока загружает первую страницу
- Инлайн-редактирование (updateNote feature) и deleteNote как отдельная feature отложены на следующую итерацию
