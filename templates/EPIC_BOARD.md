...
- [x] `updateTask/` -- инлайн-редактирование title и description на карточке
...
- [x] Отредактировать заголовок инлайн -> изменения сохранились
...
> Полный CRUD задач и подзадач реализован. Карточки отображаются на плоской сетке, подзадачи переключаются чекбоксами, инлайн-редактирование title/description работает. Vaul drawer для подтверждения удаления. Все данные сохраняются в БД.
...
- Удаление задачи -- необратимая операция, используется Vaul Drawer вместо window.confirm.
- TaskCard использует Vaul Drawer для confirmation dialog.
- Sonner Toaster интегрирован в App.tsx для уведомлений.
