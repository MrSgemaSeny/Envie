...
- [ ] **Path traversal защита:** `name` валидируется по whitelist из GET /templates.
  Прямое использование `name` как пути к файлу без валидации — уязвимость.
...
- [ ] `file_path` = UUID.randomUUID() + расширение. `original_name` только для отображения.
- [ ] `setActive` — сбрасывает active=false у всех записей, затем active=true у выбранной. В одной транзакции (@Transactional).
...
> На этой фазе тесты запрашиваются явно — AI-кодер обязан их писать
> несмотря на Rule 1 в AI_CODER_PROMPT.md (там запрет на тесты без просьбы,
> здесь — явная просьба через контекст фазы).
